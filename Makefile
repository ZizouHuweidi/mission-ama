...
.PHONY: db
db:
	docker exec -it mission_ama_db psql postgresql://admin:admin@localhost:5432/mission_ama

# Connect to the test database (you must run tests first before running this)
.PHONY: db-test
db-test:
	docker exec -it mission_ama_db psql postgresql://admin:admin@localhost:5432/mission_ama_test

# Connect to the primary cache
.PHONY: cache
cache:
	docker exec -it mission_ama_cache redis-cli

 # Connect to the test cache
.PHONY: cache-test
cache-test:
	docker exec -it mission_ama_cache redis-cli -n 1
...
run:
	clear
	go run cmd/mission_ama/main.go
