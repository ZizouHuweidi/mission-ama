...
Once that completes, you can start the application by executing `make run`. By default, you should be able to access the application in your browser at `localhost:3000`.
...
In `config/config.go`, the prefix is set as `ama` via `viper.SetEnvPrefix("ama")`. Nested fields require an underscore between levels. For example:
...
can be overridden by setting an environment variable with the name `AMA_CACHE_PORT`.
...
Sessions are provided and handled via [Gorilla sessions](https://github.com/gorilla/sessions) and configured as middleware in the router located at `pkg/routes/router.go`. Session data is stored in cookies.
...
As previously mentioned, the `Controller` acts as a base for your routes. It stores the `Container` which houses all _Services_ (_dependencies_) but also a wide array of functionality aimed at allowing you to build complex responses with ease and consistency.
...
All routes construct and _render_ a `Page`.
...
A template _component_ is located at `templates/components/messages.html`. This will render all messages of all types simply by using `{{template "messages" .}}` either within your page or layout template.
...
1. `layouts/main.html` as the base template
2. `pages/home.html` to provide the `content` template for the layout
...
type MissionForm struct {
    start_date      string `form:"start_date" validate:"required"`
    end_date    string `form:"end_date" validate:"required"`
    destination string `form:"destination" validate:"required"`
    employee    string `form:"employee" validate:"required"`
    type    string `form:"type" validate:"required"`
    external string `form:"external" validate:"required"`
...
A _component_ template is included to render metatags in `core.html` which can be used by adding `{{template "metatags" .}}` to your _layout_.
...
- The missions on the homepage/dashboard use [hx-get](https://htmx.org/docs/#targets) to fetch and page posts via AJAX.
...
To facilitate easy partial rendering for HTMX requests, the `Page` will automatically change your _Layout_ template to use `htmx.html`, which currently only renders `{{template "content" .}}`. This allows you to use an HTMX request to only update the content portion of the page, rather than the entire HTML.
...
- Include the templates `templates/layout/main.html` and `templates/pages/home.html`
...
This will use the template located at `templates/emails/welcome.html` and pass `templateData` to it.
